...
def test_timing_logs_elapsed_time(caplog):
...
def test_timing_with_operation(caplog):
...
def test_get_existing_file(sftp_client, remote_temp_dir, tmp_path, sample_text_file):
...
def test_get_binary_file(sftp_client, remote_temp_dir, tmp_path, sample_binary_file):
...
def test_get_large_file(sftp_client, remote_temp_dir, tmp_path, large_file):
...
def test_get_nonexistent_file_raises_error(sftp_client, remote_temp_dir, tmp_path):
...
def test_get_with_callback(sftp_client, remote_temp_dir, tmp_path, sample_text_file):
...
def test_put_text_file(sftp_client, remote_temp_dir, sample_text_file, ssh_client):
...
def test_put_binary_file(sftp_client, remote_temp_dir, sample_binary_file):
...
def test_put_nonexistent_local_file_raises_error(sftp_client, remote_temp_dir):
...
def test_put_with_callback(sftp_client, remote_temp_dir, sample_text_file):
...
def test_put_file_with_special_characters(
    sftp_client, remote_temp_dir, file_with_special_chars, tmp_path
):
...
def test_mkdir_single_directory(sftp_client, remote_temp_dir, ssh_client):
...
def test_mkdir_nested_directories(sftp_client, remote_temp_dir, ssh_client):
...
def test_mkdir_existing_directory_no_error(sftp_client, remote_temp_dir):
...
def test_mkdir_partial_existing_path(sftp_client, remote_temp_dir, ssh_client):
...
def test_chdir_existing_directory(sftp_client, remote_temp_dir):
    """Verify changing to existing directory works."""
    sftp_client.chdir(remote_temp_dir)
    assert sftp_client.getcwd() == remote_temp_dir


def test_chdir_nonexistent_directory_raises_error(sftp_client, remote_temp_dir):
...
def test_create_connection_success(ssh_container):
...
def test_create_connection_sets_keepalive(ssh_container):
...
def test_create_connection_invalid_host_raises():
...
def test_create_connection_wrong_credentials_raises(ssh_container):
...
def test_execute_simple_command_success(ssh_client):
    """Verify executing simple command works."""
    ssh_client.execute(["echo 'Hello World'"])


def test_execute_multiple_commands_chained(ssh_client, remote_temp_dir):
...
def test_execute_failing_command_logs_error(ssh_client, caplog):
...
def test_execute_command_with_output(ssh_client, remote_temp_dir):
    """Verify command output is captured in logs."""
    ssh_client.execute([f"echo 'test output' > {remote_temp_dir}/out.txt"])


def test_execute_stderr_captured(ssh_client, caplog):
...
def test_execute_gemseo_deserialize_run_available(ssh_client):
    """Verify gemseo-deserialize-run command exists in container."""
    ssh_client.execute(["which gemseo-deserialize-run"])


def test_open_sftp_returns_custom_client(ssh_client):
...
def test_open_sftp_functional(ssh_client, remote_temp_dir):
...
def test_open_sftp_transport_none_raises():
...
def test_open_sftp_client_none_raises():
