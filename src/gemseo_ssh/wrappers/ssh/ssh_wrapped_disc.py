...
class SSHDisciplineWrapper(MDODiscipline):
    ...
    def __init__(
        ...
        self.__local_workdir = Path(workdir_path)
        self.__remote_workdir = Path(remote_workdir)
        self.__authentification_method = authentification_method
        self._check_authentification_method()

        if transfer_inputs:
            missing_in = self.input_grammar.keys() - set(transfer_inputs)
            if missing_in:
                raise KeyError(f"Invalid transfer_inputs: {missing_in}")
        if transfer_outputs and not self.is_all_outputs_existing(transfer_outputs):
            missing_out = set(transfer_outputs) - self.output_grammar.keys()
    ...
    def _check_authentification_method(self):
        ...
        """
        if (
            self.__authentification_method == self.AuthentificationMethod.PASSWORD
        ...
        if (
            self.__authentification_method == self.AuthentificationMethod.PUBLIC_KEY
